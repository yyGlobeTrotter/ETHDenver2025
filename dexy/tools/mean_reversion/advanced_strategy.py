...
from core.indicators import (
    MeanReversionIndicators as MeanReversionCalculator,
)
...
class MeanReversionStrategy:
    ...
    def calculate_metrics(self, token_id: str, days: int = 10) -> Dict[str, Any]:
        ...
        df["percent_b"] = (df["price"] - df["lower_band"]) / (
    ...
    def backtest_strategy(
        self, token_id: str, days: int = 10, initial_capital: float = 10000.0
    ) -> Dict[str, Any]:
        """
        ...
        for i in range(self.lookback_period, len(df)):
            ...
            if position == 0 and df["buy_signal"].iloc[i]:
                ...
                df.loc[df.index[i], "holdings"] = (
                    df["capital"].iloc[i - 1] / df["price"].iloc[i]
                )
            ...
            # Check for sell signal
                ...
                df.loc[df.index[i], "capital"] = (
                    df["holdings"].iloc[i - 1] * df["price"].iloc[i]
                )
        ...
        buy_hold_return = (
            df["price"].iloc[-1] / df["price"].iloc[self.lookback_period] - 1
        ) * 100
    ...
    def plot_backtest_results(
        ...
        ax2.axhline(
            ...
            color="r",
            linestyle="--",
            label=f"Z-threshold: {self.z_threshold}",
        )
        ax2.axhline(
            y=-self.z_threshold,
            color="g",
        ...
        ax3.axhline(
            ...
            color="r",
            linestyle="--",
            label=f"Overbought: {self.rsi_overbought}",
        )
        ax3.axhline(
            y=self.rsi_oversold,
            color="g",
            linestyle="--",
            label=f"Oversold: {self.rsi_oversold}",
        )
        ax3.set_title("RSI")
        ax3.set_ylabel("RSI")
        ax3.set_ylim(0, 100)
...
def backtest_mean_reversion_strategy(
    token_id: str, days: int = 10, initial_capital: float = 10000.0
...
if __name__ == "__main__":
    ...
    message, artifact = backtest_mean_reversion_strategy.invoke(
        {"token_id": token, "days": 10, "initial_capital": 10000.0}
    )

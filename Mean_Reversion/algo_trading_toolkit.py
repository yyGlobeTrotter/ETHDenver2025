...
class AlgoTradingStrategy:
    ...
    def backtest(self, token_id: str, days: int = 365, initial_capital: float = 10000.0) -> Dict[str, Any]:
...
class MACrossoverStrategy(AlgoTradingStrategy):
    ...
    def calculate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['position'] = df['signal'].shift(1)  # Position based on previous day's signal
    ...
    def plot_backtest_results(self, results: Dict[str, Any], save_path: Optional[str] = None) -> None:
        ...
        ax2.set_title(f'Portfolio Performance (Strategy: {total_return:.2f}%, Buy & Hold: {buy_hold_return:.2f}%)')
...
@tool
def get_ma_crossover_signal(token_id: str, fast_period: int = 10, slow_period: int = 50) -> str:
    """
    ...
    if current_signal == 1:
        signal = "BUY"
        reason = f"Fast MA ({current_fast_ma:.2f}) is ABOVE Slow MA ({current_slow_ma:.2f})"
    elif current_signal == -1:
        signal = "SELL"
        reason = f"Fast MA ({current_fast_ma:.2f}) is BELOW Slow MA ({current_slow_ma:.2f})"
    else:
        signal = "HOLD"
...
def backtest_ma_crossover_strategy(token_id: str, fast_period: int = 10, slow_period: int = 50,
                                 days: int = 365, initial_capital: float = 10000.0) -> Tuple[str, Dict[str, Any]]:
    """
    ...
    try:
        # Run backtest
        results = strategy.backtest(token_id, days=days, initial_capital=initial_capital)
...
"""
        ...
        return f"Error backtesting strategy for {token_id}: {str(e)}", {
            'error': str(e)
        }

@tool
def compare_trading_strategies(token_id: str, days: int = 365, initial_capital: float = 10000.0) -> str:
    """
    ...
    try:
        ...
        mean_rev_results = mean_rev_strategy.backtest_strategy(token_id, days=days, initial_capital=initial_capital)
        
        # Strategy 2: MA Crossover (Short-term)
        ma_short_strategy = MACrossoverStrategy(fast_period=5, slow_period=20)
        ma_short_results = ma_short_strategy.backtest(token_id, days=days, initial_capital=initial_capital)
        
        # Strategy 3: MA Crossover (Medium-term)
        ma_medium_strategy = MACrossoverStrategy(fast_period=10, slow_period=50)
        ma_medium_results = ma_medium_strategy.backtest(token_id, days=days, initial_capital=initial_capital)
...
@tool
def get_optimal_strategy_parameters(token_id: str, strategy_type: str = "ma_crossover", 
                                  days: int = 365, initial_capital: float = 10000.0) -> str:
    """
        ...
        if strategy_type.lower() == "ma_crossover":
            ...
            for fast in fast_periods:
                for slow in slow_periods:
                    ...
                    backtest = strategy.backtest(token_id, days=days, initial_capital=initial_capital)
...
"""
            ...
            for lookback in lookback_periods:
                for z_threshold in z_thresholds:
                    strategy = MeanReversionStrategy(lookback_period=lookback, z_threshold=z_threshold)
                    backtest = strategy.backtest_strategy(token_id, days=days, initial_capital=initial_capital)

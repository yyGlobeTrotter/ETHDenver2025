...
class IndicatorParams(BaseModel):
    ...
    window: int = Field(default=20, description="The lookback window for indicator calculations (in days)")
...
class HistoricalDataParams(BaseModel):
    ...
    token_id: str = Field(description="The ID of the token (e.g., 'bitcoin', 'ethereum', 'solana')")
    days: int = Field(default=30, description="Number of days of historical data to fetch")
...
def get_token_bollinger_bands(token_id: str, window: int = 20, num_std: float = 2.0) -> Tuple[str, Dict[str, Any]]:
    """
    ...
    try:
        ...
        bb_data = indicators.calculate_bollinger_bands(prices, window=window, num_std=num_std)
...
"""
        ...
        raise ToolException(f"Error calculating Bollinger Bands for {token_id}: {str(e)}")
...
def get_token_indicators(token_id: str, window: int = 20) -> str:
    ...
    try:
        metrics = service.get_all_metrics(token_id, days=max(30, window*2), 
...
@tool(response_format="content_and_artifact")
def get_advanced_indicators(token_id: str, window: int = 20) -> Tuple[str, Dict[str, Any]]:
    """
    ...
    service = MeanReversionService()
    try:
        indicators = service.get_all_metrics(token_id, days=max(30, window*2), 
                                            z_window=window, rsi_window=window, 
                                            bb_window=window)
...
def get_historical_indicators(token_id: str, days: int = 30) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ToolException(f"Data not available or insufficient price history. Try a different token or time window. Error: {str(e)}")

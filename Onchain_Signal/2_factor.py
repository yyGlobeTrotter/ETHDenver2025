...
def get_historical_whale_transactions():
    ...
    for i in range(1, 31):
        ...
        response = requests.get(f"https://api.whale-alert.io/v1/transactions?date={date}&api_key={WHALE_ALERT_API_KEY}")
...
def generate_risk_signals():
    ...
    if current_whales and historical_whales:
        ...
        if deviation > 50:  # Example threshold: Whale transfers 50% higher than average
            risk_score += 3
            signal_messages.append(f"🚨 Whale transactions are {deviation:.2f}% above historical average.")
        elif deviation < -50:  # Sudden drop in whale activity
            risk_score += 1
            signal_messages.append(f"⚠️ Whale transactions are {deviation:.2f}% below historical average.")
